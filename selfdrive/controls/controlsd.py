...
class Controls:
  def __init__(self, sm=None, pm=None, can_sock=None, CI=None):
    ...
    self.enable_mads = params.get_bool("EnableMads")
    self.mads_disengage_lateral_on_brake = params.get_bool("DisengageLateralOnBrake")
    ...
    self.not_fwd_gears = [car.CarState.GearShifter.park, car.CarState.GearShifter.reverse, car.CarState.GearShifter.neutral]
    self.wrong_gear = None
  ...
  def update_events(self, CS):
    ...
    for i, pandaState in enumerate(self.sm['pandaStates']):
      ...
      if safety_mismatch or self.mismatch_counter >= 200 or self.mismatch_counter_long >= 200:
    ...
    if self.sm.rcv_frame['managerState'] and (not_running - IGNORE_PROCESSES) \
      and not Params().get_bool("ProcessNotRunningOff"):
    ...
    if self.rk.lagging and not Params().get_bool("ControlsdLaggingOff"):
    ...
    if (not self.sm.all_checks() or self.can_rcv_error) and no_system_errors and CS.canValid and not CS.canTimeout and \
      not Params().get_bool("CommIssueOff"):
  ...
  def state_control(self, CS):
    ...
    self.wrong_gear = True if CS.gearShifter in self.not_fwd_gears else False
    ...
    CC.latActive = (self.active or self.mads_ndlob) and not CS.steerFaultTemporary and not CS.steerFaultPermanent and \
                     ...
                     CS.madsEnabled and (not CS.brakePressed or self.mads_ndlob) and (not CS.belowLaneChangeSpeed or
                     (not (((self.sm.frame - self.last_blinker_frame) * DT_CTRL) < 1.0) and
                     not (CS.leftBlinker or CS.rightBlinker))) and not (self.wrong_gear and CS.seatbeltUnlatched and CS.doorOpen) and \
                     not self.events.any(ET.NO_ENTRY)
  ...
  def publish_logs(self, CS, start_time, CC, lac_log):
    ...
    if (CS.gearShifter in self.not_fwd_gears or CS.seatbeltUnlatched or CS.doorOpen) and \
      CS.madsEnabled and not CS.cruiseState.enabled and CS.standstill:
      clear_event_types.add(ET.IMMEDIATE_DISABLE)
      clear_event_types.add(ET.SOFT_DISABLE)
      clear_event_types.add(ET.USER_DISABLE)
      self.current_alert_types.append(ET.NO_ENTRY)
    elif CS.gearShifter == car.CarState.GearShifter.reverse and CS.madsEnabled and not CS.cruiseState.enabled:
      self.current_alert_types.append(ET.NO_ENTRY)
    else:
      clear_event_types.add(ET.NO_ENTRY)
    ...
    controlsState.enabled = not CS.brakePressed and (self.enabled or CS.cruiseState.enabled)
    controlsState.active = not CS.brakePressed and (self.active or CS.cruiseState.enabled)
