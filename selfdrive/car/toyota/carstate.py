...
class CarState(CarStateBase):
  def __init__(self, CP):
    ...
    self.visual_brake_lights = self.param_s.get_bool("BrakeLights")
  ...
  def update(self, cp, cp_cam):
    ...
    if self.CP.carFingerprint in RADAR_ACC_CAR:
      self.acc_type = cp.vl["ACC_CONTROL"]["ACC_TYPE"] if not self.force_sng else 1
      ret.stockFcw = bool(cp.vl["ACC_HUD"]["FCW"])
    elif self.CP.carFingerprint in TSS2_CAR:
      self.acc_type = cp_cam.vl["ACC_CONTROL"]["ACC_TYPE"] if not self.force_sng else 1
